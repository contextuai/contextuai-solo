...
Provides async MySQL database connectivity with connection pooling,
...
class MySQLConnector:
    """
    MySQL database connector with async connection pooling.
    ...
    def __init__(self):
        ...
        self.pool_config = get_pool_config("mysql")
    ...
    async def execute_query(
        ...
        """
        Execute a SQL query against MySQL database.
        ...
        # Ensure credentials have db_type set to mysql
        mysql_credentials = {**credentials, "db_type": "mysql"}

        try:
            # Get connection pool
            pool = await self.pool_manager.get_pool(persona_id, mysql_credentials)
            ...
            # Update pool metrics
            cache_key = self.pool_manager._generate_cache_key(persona_id, mysql_credentials)
    ...
    async def explain_query(
        ...
        """
        try:
            # MySQL EXPLAIN with JSON format (MySQL 5.7+)
            ...
            if result["success"] and result["rows"]:
                ...
                import json
                explain_data = result["rows"][0]
                # The JSON is in the first column
                json_str = list(explain_data.values())[0]
                if isinstance(json_str, str):
                    explain_json = json.loads(json_str)
                else:
                    explain_json = json_str
